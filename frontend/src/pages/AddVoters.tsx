...
export default function AddVoters() {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedDistrict || !selectedConstituency) {
      setUploadError("Please select District and Constituency.");
      return;
    }

    if (
      areaType === "Rural" &&
      (!selectedTc || !selectedGpu || !selectedWard)
    ) {
      setUploadError("For Rural areas, please select TC, GPU, and Ward.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to upload PDF: ${res.status} ${errorText}`);
      }

      // Parse response but don't store in variable since we don't use it
      await res.json();
      ...
      <div className="overflow-x-auto rounded-box border border-base-content/5 bg-base-100">
        ...
        <div className="flex justify-end mt-4">
          <button
            ...
            onClick={() =>
              (
                document.getElementById("add_voter_modal") as HTMLDialogElement
              )?.showModal()
            }
          >
        ...
        {isLoading ? (
          ...
          <div className="alert alert-error mx-4 my-2">
                ...
                {paginatedVoters.length > 0 ? (
                  paginatedVoters.map((w) => (
                    <tr key={w.id} className="hover:bg-base-200/50">
                      ...
                      <td className="px-3 py-2">
                        {w.gpu ? `${w.gpu.gpu_no} - ${w.gpu.gpu_name}` : "—"}
                      </td>
                      <td className="px-3 py-2">
                        {w.ward
      ...
      <dialog id="add_voter_modal" className="modal">
        <div className="modal-box">
          ...
          <form className="w-full max-w-md space-y-3" onSubmit={handleSave}>
            ...
            <div>
              <label className="label">
                <span className="label-text">Upload Voter PDF</span>
              </label>
