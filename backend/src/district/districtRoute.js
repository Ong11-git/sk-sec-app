...
districtRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const district = await createDistrict(name, code);
...
districtRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      const district = await updateDistrict(id, name, code);
      res.json({ message: "District updated successfully", district });
    } catch (error) {
      res
        .status(500)
        .json({ error: error.message || "Failed to update district" });
    }
  }
);
...
districtRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete district" });
    }
  }
);
